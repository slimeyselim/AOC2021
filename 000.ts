...
//fix all this.. should be fine then....

class ArrayObject {
    length : number;
    compressionLength? : number;
    compressionRepeated? : number;
    

    constructor(length:number, compressionRepeated:number = 0, compressionLength: number = 0) {
        
        this.length = length;
        this.compressionLength = compressionLength;
        this.compressionRepeated = compressionRepeated;
    }
}
...
class DayZero{
    RunA(input: string) {
        ...
        let currentString = "";
        let stringArray: any[] = [];
        let arrayObject : ArrayObject[] = [];
        let isRepeater : boolean = false;
        let chunkAmount : number = 0; 

        for(let i = 0; i < input.length ; i++){
            //console.log(i);
            
            if(isRepeater && input[i] !== ")") currentString += input[i];
            
            if(input[i] !== "(" && !isRepeater) currentString +=input[i];

            if(input[i] == "(" && !isRepeater) {
                stringArray.push(currentString);
                arrayObject.push(new ArrayObject(currentString.length));
                currentString = input[i];
                isRepeater = true;
            }
            
            if(isRepeater && input[i] == ")"){
                currentString += input[i];
                let getNumber = currentString.split("x");
                chunkAmount = +getNumber[0].substring(1);
                
                let compressionlength = currentString.length;
                currentString += input.substring(i+1,chunkAmount);
                let compressionNumber = chunkAmount; 
                stringArray.push(currentString);
                arrayObject.push(new ArrayObject(currentString.length,compressionlength, compressionNumber));
                currentString = "";
                isRepeater = false;
                i = i + chunkAmount;
            }  
        ...
        //TODO fix this
        arrayObject.map(x => {
            //console.log(`Length is ${x.length}, compression length is   ${x.compressionLength}, compression number is ${x.compressionNumber}`);
        });

        //just need to get the correct amount now. so other side of x number times. i.e 4725 * 11 

        //add length if compression values are blank. else multiple together and add all total together
